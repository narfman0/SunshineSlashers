...
namespace XNAnimationPipeline.Pipeline
{
    ...
    public class SkinnedModelProcessor : ContentProcessor<NodeContent, SkinnedModelContent>
    {
        ...
        */
        ...
        /// </summary>
        /// <param name="model">The model to be processed.</param>
        /// <param name="context">The content processor context.</param>
        private void ProcessMaterials(ModelContent model, ContentProcessorContext context)
        {
            ...
            foreach (ModelMeshContent modelMesh in model.Meshes)
            {
                foreach (ModelMeshPartContent meshPart in modelMesh.MeshParts)
                {
                    ...
                    if (!processedMaterials.TryGetValue(meshPart.Material, out processedMaterial))
                    {
                        processedMaterial = ProcessMaterial(meshPart.Material, context);

                        // Add the material to the processed material hash
                        processedMaterials.Add(meshPart.Material, processedMaterial);
                    }

                    // Set the processed material
                    meshPart.Material = processedMaterial;
                }
            }
        }
        */
