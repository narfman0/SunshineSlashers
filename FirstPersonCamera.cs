...
namespace SunshineSlashers
{
    public class FirstPersonCamera : GameComponent
    {
        ...
        private Vector2[] controllerSmoothingCache;

        public float CONTROLLER_X_SENSITIVITY = 22.0f;
        public float CONTROLLER_Y_SENSITIVITY = 10.0f;
        public bool CONTROLLER_Y_INVERTED = true;
...
//////////////////////////////////////////////////////////////////////
        ...
        private void UpdateInput()
        {
            ...
            // mouse look code - for windows only
#if WINDOWS
            ...
            else
            {
                ...
                // controller look code
                float deltaControllerX = CONTROLLER_X_SENSITIVITY * (0.0f - GamePad.GetState(PlayerIndex.One).ThumbSticks.Right.X);
                float deltaControllerY = CONTROLLER_Y_SENSITIVITY * (0.0f - GamePad.GetState(PlayerIndex.One).ThumbSticks.Right.Y);

                if (!CONTROLLER_Y_INVERTED)
                {
                    deltaControllerY *= -1;
                }

                smoothedMouseMovement.X = (float)deltaControllerX;
                smoothedMouseMovement.Y = (float)deltaControllerY;
                //Rotate(deltaControllerX, deltaControllerY);
